      ...
      parallel {
        stage('linux64') {
          ...
          steps {
            ...
            git fetch --tags
            make config-3rdParty -j`nproc`
            make config-OMSimulator -j`nproc`
            make OMSimulator -j`nproc`
        ...
        stage('linux64-asan') {
          ...
          steps {
            ...
            git fetch --tags
            make config-3rdParty -j`nproc`
            make config-OMSimulator ASAN=ON -j`nproc`
            make OMSimulator -j`nproc`
            '''
        ...
        stage('linux32') {
          agent {
            docker {
              image 'docker.openmodelica.org/build-deps:v1.13-i386'
              label 'linux'
              alwaysPull true
          ...
          stages {
            stage('cross-compile') {
              ...
              steps {
                ...
                make config-3rdParty CC=$CC CXX=$CXX -j`nproc`
                make config-OMSimulator -j`nproc`
                make OMSimulator -j`nproc`
            ...
            stage('test') {
              ...
              agent {
                docker {
                  image 'docker.openmodelica.org/build-deps:v1.13-arm32'
                  label 'linux-arm32'
...
void partest(cache=true, asan=false, extraArgs='') {
  sh """
